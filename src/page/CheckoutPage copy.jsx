...
export default function CheckoutPage({ selectedTickets, totalAmount, onBack, onSuccess }) {
  ...
  useEffect(() => {
    ...
    fetchTasa();
  }, []); // --- CÁLCULOS MATEMÁTICOS --- // ⚠️ REDONDEO CRÍTICO: Aseguramos 2 decimales exactos para la comparación en DB

  const montoCalculado = tasaBCV > 0 ? totalAmount * tasaBCV : 0;
  const montoEnBs = parseFloat(montoCalculado.toFixed(2)); // Función para formato "Bs. 1.200,50"
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const whatsappMessage = `🎉 ¡Felicidades, ${
        formData.nombre.split(' ')[0]
      }! Tu compra de ticket(s) ${selectedTickets.join(
        ', '
      )} ha sido *CONFIRMADA*. Tu ID de Venta es #${ventaId}. ¡Mucha suerte!`;
      ...
      console.error('Error en la transacción:', error); // Manejo de errores específicos
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-lg mt-6 mb-20">
      ...
            <h2 className="text-2xl font-bold text-gray-800 mb-6">Confirmar y Pagar</h2>      {/* RESUMEN DE COMPRA */} 
         {' '}
      ...
      <div className="mb-8 border-l-4 border-green-500 bg-green-50 p-5 shadow-sm">
        ...
        <div className="bg-white p-3 rounded border border-green-200 text-sm space-y-1">
          ...
                   {' '}
          <p>
            <strong>Teléfono:</strong> 0412-123-4567
          </p>
                   {' '}
          <p>
