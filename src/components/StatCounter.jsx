...
const StatCounter = ({ title, value, icon: Icon, color = "bg-indigo-500", progress = null, hint = "" }) => {
  return (
    <div
      className="flex items-center gap-4 px-5 py-4 rounded-full bg-white/6 backdrop-blur-sm shadow-sm hover:shadow-md transition transform duration-200 hover:-translate-y-0.5"
      role="group"
      aria-label={title}
    >
      {/* icon circle */}
      <div className={`flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center ${color} text-white shadow`}>
