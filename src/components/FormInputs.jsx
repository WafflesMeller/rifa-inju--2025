 ...
 */
export const CedulaInput = ({ value, onChange, name, label, required, id }) => {
...
export const PhoneInput = ({ value, onChange, name, label = "Teléfono", required, id }) => {
  // Valor por defecto SIN GUION
  const safeValue = value || "0412";

  // Si el valor empieza con un prefijo válido, lo separamos.
  const prefijos = ["0412", "0414", "0424", "0416", "0426", "0422"];
  
  let prefijo = prefijos.find((p) => safeValue.startsWith(p)) || "0412";
  let numero = safeValue.replace(prefijo, ""); // quitar prefijo

  const triggerChange = (newPrefijo, newNumero) => {
    onChange?.({
      target: { name: name, value: `${newPrefijo}${newNumero}` } // SIN GUION
    });
  };
      ...
      <div className="flex rounded-lg overflow-hidden">
        ...
        <div className="relative grow">
          <input
            ...
            required={required}
            onChange={(e) => {
              const val = e.target.value.replace(/\D/g, "").slice(0, 7); // solo números, max 7
...
/** SearchCedulaInput: input combinado de cédula con botón de buscar
 */
export const SearchCedulaInput = ({
  value,
  onChange,
  onSearch,
  name,
  ...
  const safeValue = value || "V-";
  const [tipo, numero] = safeValue.includes("-")
    ? safeValue.split("-")
    : ["V", ""];
      ...
      <div className="flex rounded-lg overflow-hidden shadow-sm">
        ...
        <div className="relative">
          <select
            ...
            onChange={(e) => triggerChange(e.target.value, numero)}
            className="
              h-full rounded-l-lg border border-r-0 border-gray-200 bg-gray-100 
              py-2 pl-3 pr-7 text-gray-700 font-bold 
              focus:ring-2 focus:ring-indigo-500/20
              sm:text-sm outline-none cursor-pointer appearance-none
            "
          >
            <option value="V">V</option>
            <option value="E">E</option>
        ...
        <div className="relative grow">
          <input
            ...
            placeholder="12345678"
            className="
              block w-full border border-gray-200 border-l-0
              py-2.5 pl-3 pr-3 text-gray-900 placeholder-gray-400
        ...
        <button
          ...
          className="
            ...
            px-4 sm:px-5 py-2.5 font-semibold text-sm 
